...
void q3Body::Dump(FILE* file, i32 index) const {
    ...
    switch (m_flags & (eStatic | eDynamic | eKinematic)) {
        case eStatic: fprintf(file, "\tbd.bodyType = q3BodyType( %d );\n", eStaticBody); break;

        case eDynamic: fprintf(file, "\tbd.bodyType = q3BodyType( %d );\n", eDynamicBody); break;
    ...
    fprintf(
        file, "\tbd.position.Set( r32( %.15lf ), r32( %.15lf ), r32( %.15lf ) );\n",
    ...
    fprintf(
        file, "\tbd.axis.Set( r32( %.15lf ), r32( %.15lf ), r32( %.15lf ) );\n", axis.x, axis.y,
        axis.z
    );
    ...
    while (box) {
        ...
        fprintf(
            file, "\t\tq3Vec3 xAxis( r32( %.15lf ), r32( %.15lf ), r32( %.15lf ) );\n", xAxis.x,
            xAxis.y, xAxis.z
        );
        fprintf(
            file, "\t\tq3Vec3 yAxis( r32( %.15lf ), r32( %.15lf ), r32( %.15lf ) );\n", yAxis.x,
            yAxis.y, yAxis.z
        );
        fprintf(
            file, "\t\tq3Vec3 zAxis( r32( %.15lf ), r32( %.15lf ), r32( %.15lf ) );\n", zAxis.x,
            zAxis.y, zAxis.z
        );
