...
class Renderer : public q3Render {
  public:
    ...
    void SetPenPosition(f32 x, f32 y, f32 z) override { x_ = x, y_ = y, z_ = z; }
...
static void ImImpl_RenderDrawLists(ImDrawList** const cmd_lists, int cmd_lists_count) {
    ...
    for (int n = 0; n < cmd_lists_count; n++) {
        ...
        glColorPointer(
            4, GL_UNSIGNED_BYTE, sizeof(ImDrawVert), (void*)(vtx_buffer + offsetof(ImDrawVert, col))
        );
        ...
        for (size_t cmd_i = 0; cmd_i < cmd_list->commands.size(); cmd_i++) {
            ...
            glScissor(
                (int)pcmd->clip_rect.x, (int)(height - pcmd->clip_rect.w),
...
void Keyboard(unsigned char key, int x, int y) {
    ...
    switch (key) {
        ...
        case 'p': paused = paused ? true : false; break;
        case ' ':
            paused = true;
            singleStep = true;
            break;
...
void Reshape(int width, int height) {
    ...
    glLoadIdentity();
    gluLookAt(
        Camera::position[0], Camera::position[1], Camera::position[2], Camera::target[0],
        Camera::target[1], Camera::target[2], 0.0f, 1.0f, 0.0f
    );
}
...
void InitImGui() {
    ...
    io.DeltaTime = 1.0f / 60.0f; // Time elapsed since last frame, in seconds (in this sample
                                 // app we'll override this every frame because our timestep
                                 // is variable)
    ...
    void* tex_data = stbi_load_from_memory(
        (const unsigned char*)png_data, (int)png_size, &tex_x, &tex_y, &tex_comp, 0
    );
...
int InitApp(int argc, char** argv) {
    ...
    float floats[4];
    for (i32 i = 0; i < 4; ++i) floats[i] = (float)Light::ambient[i];
    glLightfv(GL_LIGHT0, GL_AMBIENT, floats);
    for (i32 i = 0; i < 4; ++i) floats[i] = (float)Light::diffuse[i];
    glLightfv(GL_LIGHT0, GL_DIFFUSE, floats);
    for (i32 i = 0; i < 4; ++i) floats[i] = (float)Light::specular[i];
    glLightfv(GL_LIGHT0, GL_SPECULAR, floats);
    for (i32 i = 0; i < 4; ++i) floats[i] = (float)Camera::position[i];
